...
func TestMatrix_MultIKJ(t *testing.T) {
	type fields struct {
		N    int
		data [][]float64
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := A.MultIKJ(tt.args.blockSize, tt.args.B, tt.args.C); (err != nil) != tt.wantErr {
				t.Errorf("Matrix.MultIKJ() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !reflect.DeepEqual(tt.args.C, tt.truth) {
				t.Errorf("Matrix.MultIKJ() = %v, want %v", tt.args.C, tt.truth)
			}
		})
	}
}
